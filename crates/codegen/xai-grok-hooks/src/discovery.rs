...
mod tests {
    ...
    /// Plugin-only seams: dispatched to plugin sidecars, never advertised to
    /// hub clients, so they are exempt from the `blockingEvents` drift guard
    /// below even though their gates are not `Observe`.
    const PLUGIN_ONLY_SEAMS: &[HookEventName] = &[
    ...
    fn gate_events_are_the_known_set() {
        ...
        let gates: std::collections::HashSet<_> = HookEventName::ALL
            .iter()
            .map(|e| e.canonical())
            .filter(|e| !PLUGIN_ONLY_SEAMS.contains(e))
