...
/// An Intercept gate hands a whole operation to a plugin and waits for its
/// outcome, and the one seam that uses it — `start_oauth_flow` — drives an
/// interactive sign-in (authorize URL, browser round trip, token exchange).
/// The deadline therefore has to outlast a person, not a subprocess; 600s
/// matches the verification bound and, like it, fails open on timeout.
pub const DEFAULT_INTERCEPT_GATE_TIMEOUT_SECS: u64 = 600;

pub const DEFAULT_INTERCEPT_GATE_TIMEOUT_MS: u64 = DEFAULT_INTERCEPT_GATE_TIMEOUT_SECS * 1000;
...
fn default_timeout_ms(event: crate::event::HookEventName) -> u64 {
    ...
    match event.traits().gate {
        ...
        GateKind::Intercept => DEFAULT_INTERCEPT_GATE_TIMEOUT_MS,
        // Replace joins the short bound: its seams substitute a payload inline
        // on a request path (provider request/response, subagent resolution,
        // credential resolve/refresh), so a slow hook stalls the very call it
        // sits in front of.
        GateKind::Observe | GateKind::Tool | GateKind::Replace => DEFAULT_TIMEOUT_MS,
    }
}
...
fn build_one_spec(
    ...
    let (command, command_raw, url, url_raw, plugin, plugin_handler) = match handler_type {
        HandlerType::Command => {
        ...
        // Plugin handlers carry no command or URL: the sidecar invoker routes by
        // plugin name (and optional handler id).
        HandlerType::Plugin => {
