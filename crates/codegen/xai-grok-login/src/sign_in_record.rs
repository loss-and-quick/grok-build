...
//! `cached_token` method that the shell's `credential_method_id` normalizes
//! back to the interactive method that owns it). A plugin sign-in leaves no such evidence
...
pub fn load_from(grok_home: &Path) -> Option<acp::AuthMethodId> {
...
pub fn record_in(grok_home: &Path, method_id: &acp::AuthMethodId) {
    let path = path_in(grok_home);
    let record = Record {
        version: VERSION,
...
pub fn clear_in(grok_home: &Path) {
    let path = path_in(grok_home);
...
pub fn home_for(auth_manager: &crate::AuthManager) -> &Path {
    auth_manager
        .auth_json_path()
        .parent()
