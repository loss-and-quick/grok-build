...
/// Seam fixtures shared by this crate's tests and the shell's: the shell owns
/// the `/login` method ids, so its end-to-end test drives these same helpers.
...
pub mod test_support {
    ...
    use super::HookCredentialSeam;

    /// A plugin-handler spec for `plugin` on any credential `event`.
    pub fn spec_for(plugin: &str, event: HookEventName) -> HookSpec {
        HookSpec {
    ...
    pub struct RecordingInvoker {
        ...
        pub last_envelope: Arc<Mutex<Option<serde_json::Value>>>,
    }
    ...
    pub fn seam_capturing(event: HookEventName) -> (HookCredentialSeam, CapturedEnvelope) {
    ...
    pub type CapturedEnvelope = Arc<Mutex<Option<serde_json::Value>>>;

    /// The envelope the plugin received, or a panic when nothing fired.
    pub fn captured(last: &CapturedEnvelope) -> serde_json::Value {
        last.lock()
            .unwrap()
            .clone()
