...
pub fn gemini_api_events(text: &str, model: &str) -> Vec<Event> {
    scripted_to_axum(gemini_api_script(text, model))
}

/// [`gemini_api_events`] as [`SseEvent`]s for a [`crate::ScriptedResponse`].
pub fn gemini_api_script(text: &str, model: &str) -> Vec<SseEvent> {
    vec![
        SseEvent::data(
            json!({
                "candidates": [{
