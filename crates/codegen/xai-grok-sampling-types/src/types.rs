...
mod tests {
    ...
    /// Drift guard for the tool-facing effort vocabulary.
    ///
    /// `xai_tool_types` spells the accepted `Task.reasoning_effort` values as
    /// string literals because it cannot depend on this crate (the reverse edge
    /// already exists, so it would be a cycle). If a variant is added here and
    /// not there, the new level silently becomes un-selectable by an agent; if a
    /// value is added there and not here, the tool advertises a level that fails
    /// to parse. Either way this test breaks first.
    #[test]
    fn tool_facing_effort_vocabulary_matches_the_enum() {
        use std::str::FromStr;

        // Exhaustive by construction: `ReasoningEffort` is non-exhaustive-free,
        // so adding a variant without extending this array trips the length
        // assertion below.
        const ENUM_VARIANTS: [ReasoningEffort; 7] = [
            ReasoningEffort::None,
            ReasoningEffort::Minimal,
            ReasoningEffort::Low,
            ReasoningEffort::Medium,
        ...
        // Every tool-facing string parses and round-trips to itself.
        for value in xai_tool_types::REASONING_EFFORT_VALUES {
            let parsed = ReasoningEffort::from_str(value)
                .unwrap_or_else(|e| panic!("tool advertises '{value}' but it does not parse: {e}"));
            assert_eq!(
                parsed.as_str(),
                value,
                "'{value}' must round-trip to its canonical spelling"
            );
        }

        // ...and nothing the enum can express is missing from that list.
        assert_eq!(
            ENUM_VARIANTS.len(),
            xai_tool_types::REASONING_EFFORT_VALUES.len(),
            "a ReasoningEffort variant is missing from xai_tool_types::REASONING_EFFORT_VALUES"
        );
        for variant in ENUM_VARIANTS {
            assert!(
                xai_tool_types::REASONING_EFFORT_VALUES.contains(&variant.as_str()),
                "ReasoningEffort::{variant:?} is not offered to agents"
            );
        }
    }
}
