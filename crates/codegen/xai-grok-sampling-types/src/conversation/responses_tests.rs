...
fn test_tool_choice_to_responses_api() {
    ...
    let req = ConversationRequest::from_items(vec![ConversationItem::user("test")])
        .with_tools(vec![responses_test_tool()])
...
// ── Wire-legality of replayed reasoning, refusals, and tool_choice ──────────

/// A tool spec, so a request that sets `tool_choice` also has `tools` for it
/// to constrain (the conversion drops a choice with no tools).
fn responses_test_tool() -> ToolSpec {
    ToolSpec {
...
/// output item and nothing else, so `response_to_conversation_items` appends
/// an empty `Assistant`.
fn reasoning_only_turn_items() -> Vec<ConversationItem> {
    vec![
        ConversationItem::Reasoning(rs::ReasoningItem {
            ...
            summary: vec![rs::SummaryPart::SummaryText(rs::SummaryTextContent {
                text: "thinking with nothing to show for it".to_string(),
            })],
            content: None,
            encrypted_content: Some("enc_orphan".to_string()),
            status: Some(rs::OutputStatus::Completed),
        }),
        ConversationItem::Assistant(AssistantItem {
            content: "".into(),
...
/// A trailing reasoning-only turn has no item to attach to, so it must not
/// be the last thing in `input[]` — the API 400s on exactly that.
#[test]
fn test_reasoning_only_turn_does_not_end_input_with_reasoning() {
    ...
    assert_eq!(
        ...
        Some("message"),
        "input[] must not end on a reasoning item: {types:?}"
    );
    assert!(
        !types.iter().any(|t| t == "reasoning"),
        "the orphaned reasoning item must be dropped: {types:?}"
    );
}

/// Same shape mid-conversation: the next user message is not an owner either.
#[test]
fn test_reasoning_only_turn_mid_conversation_drops_reasoning() {
    let mut items = vec![ConversationItem::user("first")];
    items.extend(reasoning_only_turn_items());
    items.push(ConversationItem::user("second"));

    let types = wire_input_types(&ConversationRequest::from_items(items));

    assert!(
        !types.iter().any(|t| t == "reasoning"),
        "a reasoning item followed only by a user turn is dangling: {types:?}"
    );
}

/// The load-bearing counterpart: reasoning followed by the turn's tool calls
/// is attached and must survive.
#[test]
fn test_reasoning_kept_when_turn_emits_only_tool_calls() {
    let items = vec![
        ...
        ConversationItem::Reasoning(rs::ReasoningItem {
            ...
            summary: vec![],
            content: None,
            encrypted_content: Some("enc_2".to_string()),
            status: None,
        }),
        ConversationItem::Assistant(AssistantItem {
            content: "".into(),
...
/// encrypted-only reasoning sibling carrying `id`, and return the `reasoning`
/// entries of the resulting Responses `input[]`.
...
/// A reasoning item with an empty id must NOT reach the wire.
///
/// `rs::ReasoningItem::id` has no `skip_serializing_if`, so an empty id
/// serializes as `"id": ""` and the endpoint rejects the whole request.
/// Carrying the encrypted blob is worth nothing if the turn 400s.
#[test]
...
/// The same item with a real id survives with its content intact.
#[test]
fn test_encrypted_reasoning_with_an_id_survives() {
    let reasoning_items = encrypted_only_reasoning_on_wire("rs_hidden");

    assert_eq!(reasoning_items.len(), 1);
    assert_eq!(reasoning_items[0].id, "rs_hidden");
    assert_eq!(
        reasoning_items[0].encrypted_content,
        Some("enc_hidden_thoughts".to_string())
    );
    assert!(reasoning_items[0].summary.is_empty());
}

/// The regression this filter exists for: a history recorded against a
/// chat-completions model, replayed after the session is switched to a model
/// on the Responses backend (same provider, same credential — only
/// `api_backend` differs).
#[test]
...
/// The filter discriminates on the invalid shape, not on "is a reasoning
/// item": a Responses-native sibling keeps its id and `encrypted_content`.
#[test]
...
/// `tool_choice` with no `tools` is rejected by OpenAI-compatible endpoints
/// (`required` most loudly), so the conversion must drop it.
#[test]
fn test_tool_choice_dropped_when_no_tools_responses() {
    for choice in [
        ...
        assert!(
            ...
            "{choice:?}: tool_choice must be omitted when the request has no tools"
        );
    }
}

/// `reasoning` is sent only when an effort was requested: a model with no
/// reasoning dial rejects the parameter, and an unset effort is exactly how
/// the catalog reports that.
#[test]
fn test_responses_request_omits_reasoning_when_effort_unset() {
    ...
    let responses_req: rs::CreateResponse = (&req).into();
    assert!(
        responses_req.reasoning.is_none(),
        "no effort requested must mean no `reasoning` object at all"
    );
    ...
    let responses_req: rs::CreateResponse = (&req).into();
    let reasoning = responses_req
        .reasoning
        .expect("an effort must produce a `reasoning` object");
    assert!(reasoning.effort.is_some());
}

/// A refusal is surfaced out of band rather than folded into assistant text:
/// folding it in would make the turn look non-empty and suppress the shell's
/// refusal notice.
#[test]
fn test_refusal_part_is_extractable_and_not_assistant_text() {
    ...
    assert_eq!(
        response_refusal(&response),
        Some("I can't help with that.".to_string()),
        "the refusal explanation must be liftable out of the response"
    );
    ...
    assert!(
        ...
        "a refusal must not become assistant content, got: {assistant_text:?}"
    );
}
