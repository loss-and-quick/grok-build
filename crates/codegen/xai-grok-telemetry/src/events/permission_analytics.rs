...
pub enum PermissionPromptOutcomeDetail {
    ...
    RejectAlwaysMcpTool,
    RejectAlwaysDomain,
    /// A `permission_ask` plugin denial. Wire value predates the owner
    /// projection, so it stays `plugin_deny` rather than the snake_case name.
    #[serde(rename = "plugin_deny")]
    PolicyDeny,
    Cancelled,
    Followup,
    Error,
...
pub enum PermissionClassifierSource {
    Llm,
    Heuristic,
    /// The classifier answered but its reply would not parse, so the heuristic
    /// decided — distinct from `Heuristic`, which is the by-design fast path.
    HeuristicFallback,
    Timeout,
    TransportError,
    FastPath,
...
impl TryFrom<&str> for PermissionClassifierSource {
    ...
    fn try_from(s: &str) -> Result<Self, ()> {
        Ok(match s {
            ...
            "heuristic_fallback" => Self::HeuristicFallback,
