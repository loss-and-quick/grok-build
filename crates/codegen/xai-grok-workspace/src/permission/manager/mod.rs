        ...
        while let Some(cmd) = rx.recv().await {
            match cmd {
                ...
                PermissionCommand::Request {
                    ...
                    // Every managed-policy check inside the preflight is scoped to the
                    // requesting agent so agent-scoped rules bind only their agent; the
                    // root session (subagent_type `None`) matches unscoped and
                    // `main`-scoped rules.
                    ...
                    let (decision, outcome_str, user_prompted) = match &access {
                        AccessKind::Bash(cmd) => {
                            let prompt_outcome = tokio::select! {
                                outcome = prompter.request(&access, &tool_call_update, protected_edit, hook_ask.as_ref(), request_agent) => outcome,
                            ...
                            let decision = match prompt_outcome {
                                ...
                                // A `permission_ask` plugin denied the call: the model
                                // sees the reason and adapts (the turn is not cancelled).
                                PromptOutcome::PolicyDeny(reason) => Decision::PolicyDeny(reason),
                        ...
                        _ => {
                            ...
                            let decision = match &prompt_outcome {
                                ...
                                PromptOutcome::PolicyDeny(reason) => {
                                    Decision::PolicyDeny(reason.clone())
                                }
                            };
                            let outcome_str = effective_kind.wire_str();
                            (decision, outcome_str, true)
            ...
            .run_until(async {
                ...
                let explore = mgr
                    .request(PermissionRequest {
                        ...
                        ..PermissionRequest::new(AccessKind::Bash("rm -rf /".into()), tool_call())
                    })
                    .await
                    .decision;
                assert!(
