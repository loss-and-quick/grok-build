...
impl CompiledPolicy {
    ...
    pub fn evaluate_scoped(
        ...
        if let AccessKind::Bash(cmd) = access {
            if self.has_bash_allow_rules
                && self.bash_chain_fully_allowed(cmd, MAX_INLINE_SHELL_DEPTH, agent)
            {
    ...
    /// grants a chain segment for a different agent.
    fn bash_chain_fully_allowed(
        ...
        for parsed in &segments {
            ...
            let inner_words = norm.words;
            if !self.bash_words_allowed(inner_words, agent) {
                return false;
            }
            let shell_words: Vec<ShellWord<'_>> = inner_words.iter().map(ShellWord::from).collect();
            match shell_dash_c_script(&shell_words) {
    ...
    fn bash_words_allowed(&self, words: &[String], agent: Option<&str>) -> bool {
        if words.is_empty() {
            return false;
        }
        let cmd = words.join(" ");
