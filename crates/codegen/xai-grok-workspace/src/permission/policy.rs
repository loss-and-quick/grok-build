...
impl CompiledPolicy {
    ...
    pub fn evaluate_bash_command_policy_for_agent(
        ...
        self.evaluate_bash_command_gate(cmd, agent)
    ...
    /// Ask to the auto-mode classifier. `agent` scopes every rule match to the
    /// requesting subagent type (see [`Self::evaluate_for_agent`]).
    pub(crate) fn evaluate_bash_command_gate(
        &self,
        cmd: &str,
        agent: Option<&str>,
    ) -> Option<GateDecision> {
    ...
    fn evaluate_command_words(
        ...
        if let Some(script) = env_split_string_script(inner_words) {
            if inline_depth_remaining > 0 {
                decision = combine_gate_decisions(
                    decision,
                    self.evaluate_bash_command_segments(&script, inline_depth_remaining - 1, agent),
                );
            } else {
                decision = combine_gate_decisions(decision, Some(GateDecision::AskFailClosed));
            }
    ...
    pub fn evaluate(&self, access: &AccessKind) -> Option<Decision> {
        self.evaluate_scoped(access, None, None)
    }

    /// Agent-scoped variant of [`Self::evaluate`]. A rule carrying a non-empty
    ...
    pub fn evaluate_for_agent(&self, access: &AccessKind, agent: Option<&str>) -> Option<Decision> {
        self.evaluate_scoped(access, None, agent)
    }
    ...
    pub fn evaluate_with_cwd(&self, access: &AccessKind, cwd: Option<&Path>) -> Option<Decision> {
        self.evaluate_scoped(access, cwd, None)
    }

    /// The full evaluation: both the cwd anchor of [`Self::evaluate_with_cwd`]
    /// and the agent scope of [`Self::evaluate_for_agent`]. The two are
    /// independent filters — a rule must clear its agent scope *and* match the
    /// cwd-joined path — so a caller that knows both must pass both, or an
    /// agent-scoped path rule silently stops binding.
    pub fn evaluate_scoped(
        ...
        self.evaluate_with_cwd_details(access, cwd, agent).0
    }

    /// Lexical decision plus whether an unresolvable native symlink forces a
    /// prompt. `agent` scopes every rule match to the requesting subagent type.
    pub(crate) fn evaluate_with_cwd_details(
        ...
        let lexical = self.evaluate_lexical_with_cwd(access, cwd, agent);
        ...
        match follow_absolute_symlink(&raw_absolute, &lexical_abs) {
            SymlinkFollow::Target(resolved) => {
                let resolved_decision = match self.evaluate_lexical_with_cwd(
                    &access_with_path(access, resolved),
                    cwd,
                    agent,
                ) {
    ...
    /// Deliberately unscoped by agent: this only ever forces a prompt, so
    /// widening it past the requesting agent's rules can never grant access.
    ...
    /// Lexical rule match only, scoped to `agent`. Callers re-check symlink
    /// targets through this so follow cannot recurse.
    ...
    pub(crate) fn narrow_allow_authorizes(&self, access: &AccessKind) -> bool {
        ...
        self.has_bash_allow_rules
            && self.bash_chain_fully_allowed(
                cmd,
                MAX_INLINE_SHELL_DEPTH,
                // Main-session scope: this gate only runs after the agent-scoped
                // evaluation already returned Allow, and its callers carry no
                // agent, so an agent-scoped rule must not widen it here.
                None,
...
mod tests {
    ...
    #[test]
    fn agent_scoped_bash_allow_does_not_grant_other_agents() {
        // The conjunctive bash-allow path is agent-scoped too: an allow bound to
        // `explore` must not grant the root session or another agent.
        let policy = CompiledPolicy::new(PermissionConfig::new(vec![agent_bash_rule(
            RuleAction::Allow,
            "ls*",
            &["explore"],
        )]));
        let ls = AccessKind::Bash("ls -la".into());
        assert!(matches!(
            policy.evaluate_for_agent(&ls, Some("explore")),
            Some(Decision::Allow)
        ));
        assert!(policy.evaluate_for_agent(&ls, Some("plan")).is_none());
        assert!(policy.evaluate_for_agent(&ls, None).is_none());
    }
    ...
    fn unresolvable_symlink_with_restrictions_asks() {
        ...
        for (tool, access) in [
            ...
            let (decision, fail_closed) =
                restricted.evaluate_with_cwd_details(&access, Some(ws.path()), None);
            assert!(
                matches!(decision, Some(Decision::Ask)),
        ...
        let (open_decision, open_fail) = open.evaluate_with_cwd_details(
            &AccessKind::Read(Some("a".into())),
            Some(ws.path()),
            None,
        );
        assert!(
