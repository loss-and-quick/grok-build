...
impl GatePreflight {
    ...
    pub(crate) fn evaluate(
        ...
        let (direct, native_symlink_fail_closed) = match policy {
            Some(policy) => policy.evaluate_with_cwd_details(access, Some(cwd), agent),
            None => (None, false),
        };
        let (bash_command, shell_file) = match (policy, access) {
            (Some(policy), AccessKind::Bash(cmd)) => (
                ...
                policy.evaluate_shell_file_access_gate(cmd, cwd, agent),
            ),
            _ => (None, None),
        };
...
mod tests {
    ...
    fn preflight_reports_gate_state_coherently() {
        ...
        let deferred =
            GatePreflight::evaluate(Some(&policy), &bash("echo \"$(date)\""), cwd, true, None);
        ...
        let ask_mode =
            GatePreflight::evaluate(Some(&policy), &bash("echo \"$(date)\""), cwd, false, None);
        ...
        let inert = GatePreflight::evaluate(None, &bash("echo hi"), cwd, true, None);
    ...
    fn native_unresolvable_symlink_forces_shell_prompt() {
        ...
        let preflight = GatePreflight::evaluate(Some(&restricted), &access, ws.path(), false, None);
        assert!(
        ...
        let grep_preflight =
            GatePreflight::evaluate(Some(&grep_only), &grep_access, ws.path(), false, None);
        assert!(matches!(
        ...
        let open_preflight = GatePreflight::evaluate(Some(&open), &access, ws.path(), false, None);
