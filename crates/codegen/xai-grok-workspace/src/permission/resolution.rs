...
///
/// `project_trusted` gates the project-tier sources exactly as in
/// [`resolve_permission_config_with_fallback`]; agent-definition rules are a
/// separate tier and are folded regardless (the caller decides which agent
/// definitions are discoverable).
pub async fn resolve_permission_config_with_agents(
    cwd: &Path,
    project_trusted: bool,
    agents: &[AgentPermissionRules],
) -> Option<PermissionConfig> {
    resolve_permissions_with_provenance_and_agents(cwd, project_trusted, agents)
        .await
        .map(|r| r.config)
}

/// [`resolve_permission_config_with_agents`] with the always-approve pin supplied by the caller,
/// for the same reason [`resolve_permission_config_with_fallback_pinned`] takes it: a session spawn
/// reads the pin once and every consumer of it must agree.
pub async fn resolve_permission_config_with_agents_pinned(
    cwd: &Path,
    project_trusted: bool,
    agents: &[AgentPermissionRules],
    yolo_lock: Option<&YoloPolicyLock>,
) -> Option<PermissionConfig> {
    let mut inputs = ResolveInputs::live(project_trusted, yolo_lock.cloned());
    inputs.agent_rules = agents
        .iter()
        .flat_map(agent_definition_sourced_rules)
        .collect();
    resolve_permissions_with_provenance_inner(cwd, inputs)
        .await
        .map(|r| r.config)
}
...
/// `None` when the config names a tool filter this build does not know.
///
/// The config mirror has no `WebSearch` variant, so every variant it *does*
/// have is expressible here. It is `#[non_exhaustive]` upstream, though, so a
/// wildcard is required even though its own deserializer rejects unknown
/// strings — meaning the fallback is unreachable from real user config.
/// Dropping the rule is the fail-closed reading: a filter we cannot interpret
/// has an unknown scope, and applying it under a guessed scope could widen an
/// `allow` rule to tools the user never named.
fn config_tool_filter(tool: xai_grok_config_types::permission::ToolFilter) -> Option<ToolFilter> {
    ...
    Some(match tool {
        ...
        C::AgentMessage => ToolFilter::AgentMessage,
        unknown => {
            tracing::warn!(
                ?unknown,
                "permission rule names an unknown tool filter; dropping the rule"
            );
            return None;
        }
    })
...
pub async fn resolve_permissions_with_provenance_and_agents(
    ...
    let mut inputs = ResolveInputs::live(project_trusted, yolo_policy_lock());
    inputs.agent_rules = agents
        .iter()
        .flat_map(agent_definition_sourced_rules)
        .collect();
