...
impl CompiledPolicy {
    ...
    /// `agent` scopes rule matching to the requesting subagent type.
    ...
    fn evaluate_shell_path(
        ...
        let escalate =
            |access: &AccessKind| match self.evaluate_lexical_with_cwd(access, rule_cwd, agent) {
...
mod tests {
    ...
    fn shell_gate_matches_cwd_relative_rules_on_absolute_operands() {
        ...
        assert!(matches!(
            deny.evaluate_shell_file_access_gate(
                "cd /tmp && cat /work/src/secret.txt",
                cwd(),
                None
            ),
            Some(GateDecision::Reject(_))
