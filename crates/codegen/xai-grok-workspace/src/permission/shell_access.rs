...
impl CompiledPolicy {
    ...
    pub fn evaluate_shell_file_access_for_agent(
        ...
        self.evaluate_shell_file_access_gate(cmd, cwd, agent)
    ...
    /// Ask to the auto-mode classifier. `agent` scopes rule matching to the
    /// requesting subagent type.
    pub(crate) fn evaluate_shell_file_access_gate(
        ...
        self.evaluate_shell_file_access_inner(cmd, cwd, MAX_INLINE_SHELL_DEPTH, false, false, agent)
    }
    ...
    fn evaluate_shell_file_access_inner(
        ...
        for redirect in shell_redirect_targets(root, cmd) {
            ...
            if let Some(path) = redirect.path {
                ...
                decision = combine_gate_decisions(
                    decision,
                    self.evaluate_shell_path(&path, cwd, redirect.mode, path_cwd_unpinned, agent),
                );
            }
        }
        ...
        for invocation in &invocations {
            ...
            for (path, mode) in special_file_operands(&program_lower, &literal_words) {
                ...
                decision = combine_gate_decisions(
                    decision,
                    self.evaluate_shell_path(&path, cwd, mode, invocation_cwd_unpinned, agent),
                );
            }
            if program_lower == "dd" {
                continue;
            ...
            for &token in &candidates {
                ...
                for &mode in modes {
                    decision = combine_gate_decisions(
                        decision,
                        self.evaluate_shell_path(token, cwd, mode, invocation_cwd_unpinned, agent),
                    );
                }
            }
    ...
    fn evaluate_shell_path(
        ...
        let escalate = |access: &AccessKind| match self.evaluate_scoped(access, rule_cwd, agent) {
...
mod tests {
    ...
    fn shell_file_gate_distinguishes_ask_provenance() {
        ...
        assert!(matches!(
            deny.evaluate_shell_file_access_gate("cat .env", cwd(), None),
    ...
    fn shell_gate_matches_cwd_relative_rules_on_absolute_operands() {
        ...
        assert!(matches!(
            deny.evaluate_shell_file_access_gate("cat /work/src/secret.txt", cwd(), None),
            Some(GateDecision::Reject(_))
        ));
        // An absolute operand is cwd-independent, so it stays covered even
        // after a `cd` unpins the working directory.
        assert!(matches!(
            deny.evaluate_shell_file_access_gate("cd /tmp && cat /work/src/secret.txt", cwd(), None),
            Some(GateDecision::Reject(_))
        ));
        // Outside the working directory the rooted rule stays silent.
        assert_eq!(
            deny.evaluate_shell_file_access_gate("cat /elsewhere/src/secret.txt", cwd(), None),
            None
        );
    }
