...
impl xai_tool_runtime::Tool for TaskOutputTool {
    ...
    async fn run(
        ...
        if ids.len() == 1 {
            ...
            return self
                .run_single_task(id, input.timeout_ms, input.wait_for_event, &ctx, resources)
                .await;
        }
...
mod tests {
    ...
    async fn a_silent_monitor_returns_at_the_budget() {
        ...
        assert!(
            result.output.contains("Waited the requested")
                && result.output.contains("It is still working"),
            "{}",
            result.output
        );
    }
