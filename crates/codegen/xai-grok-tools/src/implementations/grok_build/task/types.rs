...
pub enum ModelOverrideProvenance {
    ...
    /// carrying the selection mode its tool schema advertised.
    ///
    /// The `task` tool never reaches this variant with a slug — it rejects a
    /// model-facing `model` argument outright, because a subagent type already
    /// pins its model. So `Tool` reads as "untrusted caller", not "the model
