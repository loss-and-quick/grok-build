...
use super::coordinator_state::{
    ...
    DisplacedCompletedChild, ForegroundChild, InternalEvent, ListRequest, ParentReportReply,
    PendingChild, ProgressFuture, ProgressTarget, ReplyFuture, ResolvedParentReport, TaggedFuture,
    active_summary, background_at_deadline, background_if_caller_gone, completed_snapshot,
    hand_off_to_background, sleep_until, workflow_outstanding,
};
use super::types::{
    ActiveAgentMessageOutcome, AgentAddress, HandedOffForegroundSubagent, ParentMessageOutcome,
...
pub use super::coordinator_state::{
    ...
    MAX_ACTIVE_MESSAGE_ADMISSIONS_PER_CHILD, MAX_COMPLETED_ENTRIES, ParentReportDelivery,
    ...
    pub async fn run(mut self) {
        ...
        loop {
            ...
            tokio::select! {
                ...
                Some((respond_to, descriptors)) = self.type_lists.next(), if !self.type_lists.is_empty() => {
    ...
    fn handle_command(&mut self, command: SubagentEvent) {
        match command {
            ...
            SubagentEvent::ListTypes(request) => {
                self.type_lists.push(ReplyFuture {
                    future: Box::pin(self.runner.list_types(request.parent_session_id)),
                    respond_to: Some(request.respond_to),
                });
            }
