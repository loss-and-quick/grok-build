...
pub enum SendSubagentMessageDelivery {
    ...
    Steer,
    /// Hold it in the subagent's queue: delivered into its current turn at the
    /// next response boundary, or as its next turn if the current one ends.
    Queue,
...
impl xai_tool_runtime::Tool for SendSubagentMessageTool {
    ...
    async fn run(
        ...
        let (depth, max_depth, backend, sender) = {
        ...
        let outcome = if let Some(sender) = sender {
            ...
            // may steer through the coordinator are exactly the agents that may
            // spawn — the same `depth < max` test the spawn path makes for
            // `allow_nested_subagents`. Upstream gates this path on `depth == 0`;
            // at the default max depth of 1 the two agree, and they part only where
            // nesting is actually allowed. A depth-1 agent owns children of its own
            // and has no other way to correct one it already started.
            if depth >= max_depth {
                return Ok(SendSubagentMessageOutput::Unsupported);
            }
