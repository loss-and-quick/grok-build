...
mod tests {
    ...
    fn format_subagent_completion_failure_carries_the_resume_handle() {
        ...
        let msg = format_subagent_completion(&c, Some("get_task_output"), None, None);
        assert!(msg.contains("<subagent_resumable>"), "{msg}");
    ...
    fn format_subagent_completion_failure_without_work_withholds_the_handle() {
        let mut result = test_result("sub-stillborn", false);
        result.tool_calls = 0;
        result.turns = 0;
        let c = summarize(&test_request("sub-stillborn"), &result);
        let msg = format_subagent_completion(&c, Some("get_task_output"), None, None);
        assert!(
    ...
    fn between_turn_completions_point_a_failure_at_resume() {
        let msg = format_between_turn_completions(
            &[
                make_subagent_completion("sub-ok", true),
                make_subagent_completion("sub-fail", false),
            ],
            Some("get_task_output"),
            None,
            None,
        ...
        let resumable = msg
            .split("<subagent_resumable>")
            .skip(1)
            .collect::<Vec<_>>();
        assert_eq!(resumable.len(), 1, "{msg}");
        assert!(resumable[0].contains("subagent_id: sub-fail"), "{msg}");
    }
    #[test]
