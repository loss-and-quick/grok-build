...
fn write_summary(
    ...
    // Built as JSON: the agent selection is crate-private, so a struct literal
    // cannot name it from here; deserializing also checks the shape still loads.
    ...
    .expect("benchmark summary deserializes");
