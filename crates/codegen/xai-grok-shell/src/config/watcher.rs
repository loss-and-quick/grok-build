...
impl ProjectDiscoveryWatcher {
    ...
    /// opened it, and subscribe to its changes. `None` when that root's `.grok` is grok home itself:
    /// every grok process writes there, and the skills watcher already owns it.
    pub(crate) fn shared(
        cwd: &Path,
        grok_home: &Path,
    ) -> Option<(Arc<Self>, broadcast::Receiver<DiscoveryChange>)> {
...
mod tests {
    ...
    fn project_discovery_watcher_skips_project_grok_equal_to_grok_home() {
        ...
        assert!(ProjectDiscoveryWatcher::shared(&cwd, &project_grok).is_none());
        assert!(ProjectDiscoveryWatcher::shared(&cwd, &project.join("other-home")).is_some());
    }
    ...
    fn project_discovery_watcher_ignores_unclassified_files_under_project_grok() {
        ...
        let (_w, mut rx) = ProjectDiscoveryWatcher::shared(project, &project.join("other-home"))
    ...
    fn project_discovery_watcher_is_shared_per_root() {
        ...
        let (first, _first_rx) =
            ProjectDiscoveryWatcher::shared(root, &root.join("other-home")).unwrap();
