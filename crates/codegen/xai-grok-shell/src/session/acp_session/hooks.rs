...
/// An Intercept gate waits on a whole operation, up to and including an
/// interactive sign-in, so it gets the same person-scale bound the dispatcher
/// gives it.
const CLIENT_INTERCEPT_GATE_TIMEOUT: Duration =
    Duration::from_secs(xai_grok_hooks::config::DEFAULT_INTERCEPT_GATE_TIMEOUT_SECS);

/// Mirrors `xai_grok_hooks::config::default_timeout_ms`, so a gate dispatched
/// to the client waits as long as the same gate dispatched to a local hook.
fn default_client_gate_timeout(gate: xai_grok_hooks::event::GateKind) -> Duration {
    ...
    match gate {
        ...
        GateKind::Intercept => CLIENT_INTERCEPT_GATE_TIMEOUT,
        // Replace substitutes a payload inline on a request path, so it keeps
        // the short bound: a slow hook stalls the call it sits in front of.
        GateKind::Observe | GateKind::Tool | GateKind::Prompt | GateKind::Replace => {
            CLIENT_HOOK_TIMEOUT
        }
    }
}
