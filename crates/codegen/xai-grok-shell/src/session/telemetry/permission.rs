...
/// `plugin_tool` is the session's answer to which side of the shared `<owner>__<tool>` name shape the call came from,
/// so the external `tool_decision` name reduction agrees with `tool_result`.
...
mod permission_analytics_tests {
    ...
    fn renamed_agent_message_keeps_canonical_product_and_external_identity() {
        ...
        let payload = permission_decision_payload(
            ...
            Some(&event),
            resolved,
            // A built-in tool, not an `<owner>__<tool>` plugin call.
            false,
        );
