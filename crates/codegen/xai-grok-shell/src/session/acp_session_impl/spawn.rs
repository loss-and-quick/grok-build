...
) -> Result<(SessionInitResult, tokio::sync::oneshot::Receiver<()>), xai_grok_agent::AgentBuildError>
{
    ...
    let (permissions, permission_events_rx, deny_read_globs) = async {
        ...
        if let Some(handle) = inherited_permission_handle {
            ...
            let agent_permission_rules: Vec<permission_resolution::AgentPermissionRules> =
                xai_grok_agent::discovery::discover(tool_context.cwd.as_path())
                    .into_iter()
                    .filter(|def| !def.permissions.is_empty())
                    .map(|def| permission_resolution::AgentPermissionRules {
                        agent: def.name,
            ...
            let mut permission_config = permission_resolution::resolve_permission_config_with_agents_pinned(
                    tool_context.cwd.as_path(),
                    project_trusted,
