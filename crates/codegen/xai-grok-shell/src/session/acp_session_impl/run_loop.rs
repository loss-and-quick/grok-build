...
///
/// Both halves live on `SessionActor` (see `stop_gate.rs`): they carry this
/// fork's subagent suppression and read the dispatcher's
/// `ObserveDispatchResult`, so the session-end payload is built in one place.
        ...
        tokio::select! {
                ...
                maybe_cmd = cmd_rx.recv() => {
                    ...
                    match cmd {
                        ...
                        SessionCommand::EmitPluginPanel { plugin, view_model } => {
                            session
                                .send_xai_notification(XaiSessionUpdate::PluginPanel {
                                    plugin,
                                    view_model,
                                })
                                .await;
                        }
                        SessionCommand::ClosePluginPanel { plugin, panel_id } => {
                            session
                                .send_xai_notification(XaiSessionUpdate::PanelClosed {
                                    plugin,
                                    id: panel_id,
                                })
                                .await;
                        }
                        SessionCommand::PanelAction {
                            plugin,
                            panel_id,
                            button_id,
                            inputs,
                            ...
                            let delivered = if let Some(host) = &session.plugin_host {
                                host.deliver_panel_action(
                                    &plugin,
                                    xai_grok_plugin_protocol::PanelActionParams {
                                        panel_id,
                                        button_id,
                                        inputs,
                                    },
                        ...
                        SessionCommand::StartPluginOauthFlow {
                            ...
                            let s = session.clone();
                            tokio::task::spawn_local(async move {
                                let ok = match s.build_credential_seam() {
                                    Some(seam) => seam
                                        .start_oauth_flow(
                                            &reason,
                                            Some(&plugin),
