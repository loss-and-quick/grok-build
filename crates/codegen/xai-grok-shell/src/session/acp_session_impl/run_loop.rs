...
if let Some(ref tp)
            ...
            SessionCommand::StartPluginOauthFlow { plugin, account, reason, respond_to }
            => { let s = session.clone(); tokio::task::spawn_local(async move { let ok =
            match s.build_credential_seam() { Some(seam) => seam.start_oauth_flow(&
            reason, Some(& plugin), account.as_deref()). await .is_some(), None => false,
            }; let _ = respond_to.send(ok); }); } SessionCommand::ReloadPlugins {
