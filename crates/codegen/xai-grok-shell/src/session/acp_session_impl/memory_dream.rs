...
impl SessionActor {
    /// Re-register the memory tools on the tool bridge.
    ///
    ...
    pub(super) async fn register_memory_tools(
        ...
        use xai_grok_tools::implementations::memory::{
            MEMORY_DELETE_TOOL_NAME, MEMORY_GET_TOOL_NAME, MEMORY_SEARCH_TOOL_NAME,
        ...
        bridge
            .register_mcp_tools(
                MEMORY_DELETE_TOOL_NAME.to_owned(),
    ...
    pub(super) async fn handle_rewrite_memory_note(
        ...
        let result = match sampling_client.api_backend() {
            ...
            crate::sampling::ApiBackend::Gemini => {
                return Err("rewrite unsupported: Gemini backend is not yet supported".to_string());
            }
        };
