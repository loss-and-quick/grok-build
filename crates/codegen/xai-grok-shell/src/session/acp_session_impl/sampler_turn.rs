...
impl SessionActor {
    ...
    pub(crate) async fn run_turn_via_sampler(
        ...
        if !budget.can_wait() && max_attempts <= 1 {
            // Nothing will send this request a second time — no paced wait and no
            // failover hop — so move it into the sampler instead of deep-cloning the
            // whole message history on every main-session turn.
        ...
        loop {
            ...
            if let RateLimitWaitDecision::Wait { attempt, backoff } = decision {
                ...
                // (parked turns skip it — see above).
                if !park.is_parked() {
                    self.prepare_sampler_for_turn().await;
                }
