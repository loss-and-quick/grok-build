...
use wait_interrupt::{
    ...
    interrupted_wait_tool_result, interrupted_wait_tool_result_with_msg,
    record_interruptible_wait_outcome, wait_for_wait_interrupt, wait_task_ids_from_args,
};
#[derive(Default)]
...
impl SessionActor {
    ...
    pub(crate) async fn prepare_tool_call(
        ...
        // bridge must not wait on (or be failed by) MCP server readiness.
