...
impl SessionActor {
    ...
    ///
    /// Temperature, the visible-output budget and the reasoning effort come
    /// from the `[prompt_suggestions]` config, resolved against the wire model
    /// the route names rather than against the pin: a model that rejects an
    /// explicit `reasoningEffort` is sent none, and a reasoning model gets the
    /// extra budget its thinking needs so a small cap cannot silently empty the
    /// answer. Output is filtered through
    /// [`prompt_suggest::sanitize_suggestion`]; any failure returns `None`.
    pub(super) async fn handle_suggest_prompt(
        ...
        // The suggestion budget and reasoning effort are resolved against the
        // wire model the route actually names, not against the pin: a catalog
        // key can front an entry whose endpoint serves a different id, and the
        // effort a model rejects is a 400, not a smaller answer.
        ...
        let suggestion = match prompt_suggest::sanitize_suggestion(&raw) {
            ...
            // but a filter guarantees it). Logged apart from `FetchedEmpty`:
            // the model did answer, this side suppressed it, and the two say
            // different things about the prompt's quality.
