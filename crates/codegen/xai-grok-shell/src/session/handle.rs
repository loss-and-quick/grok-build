...
pub struct SessionHandle {
    ...
    /// The session's TS plugin sidecar host (snapshot from spawn time; `None`
    /// when the session has no sidecar plugins). Lets pre-spawn seams that run
    /// outside the session actor — the coordinator's `subagent_resolve`
    /// dispatch — reach the same sidecars as in-actor dispatches, and is what a
    /// subagent spawn hands its child so the whole session tree shares one
    /// sidecar process per plugin.
    pub plugin_host: Option<std::sync::Arc<xai_grok_plugin_host::PluginHost>>,
