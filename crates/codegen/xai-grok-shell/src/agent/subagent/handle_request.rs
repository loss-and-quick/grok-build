...
async fn apply_subagent_resolve_hook(
    ...
    let run_ctx = xai_grok_hooks::runner::RunContext {
        ...
        workspace_root: &cwd,
        // The parent session's scope, the same one the child's tool context
        // inherits below, so a command hook fired during resolve is reaped
        // when that session closes.
        process_scope: ctx.process_scope.clone(),
        plugin_invoker: ctx
            .parent_plugin_host
            .clone()
...
pub(crate) async fn run_shell_child(
    ...
    // Built only to fail the spawn early on a config the sampler rejects: the
    // session title no longer rides a client opened here (it routes through
    // the aux failover loop), so nothing downstream holds this one.
