...
pub(crate) use spawn::{
    ...
    SubagentProgress, emit_subagent_notification, spawn_subagent_coordinator,
    ...
    if let Some(ref chat_state) = ctx.parent_chat_state {
        if let Some(cfg) = chat_state.get_sampling_config().await {
            ...
            let auth_scheme = crate::agent::config::try_resolve_model_credentials(&cfg.model, None)
                .map(|r| r.auth_scheme)
                .unwrap_or_default();
            // A parent routed through a [[provider]] hands its transport and
            // wire-shape facts down, so the subagent reaches the same endpoint
            // and speaks the dialect that endpoint accepts.
            let parent_facts =
                crate::agent::config::resolve_model_auth_facts_and_provider(&cfg.model).0;
            let proxy = parent_facts.proxy;
...
/// `str::cmp` for stable rendering. Backs [`available_subagent_descriptors`],
/// which the coordinator reaches through `ChildRunner::list_types` for the
/// plugin `agent_list` RPC. `validate_subagent_type`'s `Unknown` reply builds
/// its own list inside `xai_grok_subagent_resolution::validate_agent_name`.
