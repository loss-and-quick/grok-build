...
use xai_grok_tools::implementations::grok_build::task::coordinator::{
    ...
    ChildRunner, CoordinatorConfig, LocalBoxFuture, ParentReportDelivery, SendBoxFuture,
...
impl ChildControl for SnapshotProbeControl {
    ...
    fn message_parent(&self, text: String) -> Self::ParentReportFuture {
        self.runtime.message_parent(text)
    }
...
impl ChildRunner for SnapshotProbeRunner {
    ...
    type ListTypesFuture = LocalBoxFuture<Vec<SubagentTypeDescriptor>>;
                ...
                .started(StartedChild {
                    ...
                    control: SnapshotProbeControl {
                        runtime: ShellChildRuntime {
                            ...
                            // The probe drives the active-message path only;
                            // there is no parent actor to report up to.
                            parent_cmd_tx: None,
