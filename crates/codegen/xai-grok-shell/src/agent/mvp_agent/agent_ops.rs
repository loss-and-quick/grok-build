...
/// post-plan-approval agent switch, via [`crate::agent::remote_config::ModelsManager`]
...
impl MvpAgent {
    ...
    pub(crate) async fn drive_plugin_oauth(
        ...
        let cmd_tx = self
            ...
            .resident_ids()
            .first()
            .and_then(|id| self.session_registry.resident_handle(id))
    ...
    /// The keys are `x.ai/sessionConfig`, `x.ai/sessionDetail`, `x.ai/schedulerBackgroundLoops`,
    /// and `x.ai/memoryMode`.
