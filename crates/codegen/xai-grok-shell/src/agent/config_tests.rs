...
fn finalize_image_describe_sampler_some_stamps_session_fields() {
    ...
    let (model, cfg) = finalize_aux_sampler_config(Some(aux), &active, Some("cli".into()), Some(7));
...
fn session_on(base_url: &str, resolver: xai_grok_sampler::SharedBearerResolver) -> SamplerConfig {
    SamplerConfig {
...
fn bare_slug_resolves_to_the_locally_declared_entry_through_every_resolver() {
    ...
    let (default_key, default_entry, _) =
        crate::agent::remote_config::resolve_default_model(&cfg, &catalog, true);
...
fn provider_reasoning_efforts_reach_acp_model_meta() {
    ...
    let menu =
        xai_grok_sampling_types::parse_reasoning_efforts_meta(meta.as_ref()).expect("menu in meta");
    assert_eq!(
