...
use xai_grok_sampling_types::{
    ...
    ThinkingDialect, reasoning_effort_meta_value, reasoning_efforts_meta_value,
};
...
///
/// `pub` rather than `pub(crate)`: the pager's resolved-catalog panel replays
/// this exact resolution so what it shows is what the sampler will send.
pub fn resolve_model_list(
    ...
    for (key, model_override) in &cfg.config_models {
        ...
        // The explicit-knob sets read the table as its provider defaults leave
        // it; the entry itself comes from `apply_config_model_override`, which
        // the `[[provider]]` re-apply below shares.
...
pub fn find_by_slug<'a>(
    ...
    // `has_model_id`, not `entry.model ==`: a per-effort variant id is a name the
    // backend answers to, so a slug that only appears in `variants` still lands
    // on the entry that sends it.
    models
        .iter()
        .rev()
        .find(|(_, entry)| entry.info.has_model_id(slug))
}
...
impl ModelEntry {
    ...
    ///
    /// `pub` rather than `pub(crate)`: the pager's resolved-catalog panel reads
    /// it to show whether an entry carries its own credential.
...
// Not `Copy`: the provider-derived fields below carry owned strings.
#[derive(Clone)]
...
/// known to serve.
pub(crate) fn finalize_aux_sampler_config(
...
/// Resolve `model_id` to a full [`SamplerConfig`] against `models`.
///
/// The seam `build_failover_config` re-issues a turn through: a catalog hit
/// brings its own base URL, backend and credentials, so a `[[model_fallbacks]]`
/// hop lands on the target provider rather than replaying the origin's
/// transport. `fallback_entry` covers the same-provider model swap, where the
/// target is not in the catalog at all.
