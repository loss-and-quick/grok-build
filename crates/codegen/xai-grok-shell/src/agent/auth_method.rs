...
mod tests {
    ...
    /// Lives here rather than beside the seam because the method ids are the
    /// shell's; the seam and its recording plugin come from the login crate.
    #[tokio::test]
    async fn account_picked_in_login_reaches_the_plugin() {
        use crate::auth::credential_seam::PluginCredentialSeam;
        use crate::auth::credential_seam::test_support::{captured, seam_capturing};
