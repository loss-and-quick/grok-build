...
pub fn apply_definition_runtime_defaults(
    ...
    if runtime.reasoning_effort.is_none() {
        // `EffectiveRuntimeConfig::reasoning_effort` is the typed enum here, so
        // the definition's effort is parsed once and an unrecognized value is
        // dropped rather than carried as a raw string.
        runtime.reasoning_effort = definition
            .effort
            .and_then(|effort| <&str>::from(effort).parse().ok());
    }
