...
impl LockOptions {
    ...
    /// Unix permission bits for a lock file this call creates (e.g. `0o600` for a file that
    /// names a process a stranger could signal). Default `None`: the ambient umask decides. A
    /// file that already exists keeps its mode. Ignored off unix.
    pub fn with_create_mode(mut self, mode: u32) -> Self {
        self.create_mode = Some(mode);
        self
    }

    pub fn create_mode(&self) -> Option<u32> {
        self.create_mode
    }
