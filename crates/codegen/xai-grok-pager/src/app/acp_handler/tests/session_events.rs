#![cfg_attr(rustfmt, rustfmt::skip)]
    ...
    fn child_compact_completed_updates_subagent_info() {
        ...
        let changed = handle_child_session_notification(update, child_sid, &mut agent, &oauth_auth(), None);
        assert!(changed);
    ...
    fn child_compact_started_refreshes_context_used() {
        ...
        let _ = handle_child_session_notification(update, child_sid, &mut agent, &oauth_auth(), None);
    ...
    fn child_notification_without_view_returns_false() {
        ...
        let changed = handle_child_session_notification(update, "unknown-child", &mut agent, &oauth_auth(), None);
        assert!(!changed);
    }
    ...
    fn child_unknown_event_returns_false() {
        let mut agent = make_agent(Some("root-sess"));
        let update = XaiSessionUpdate::MemoryFlushStarted;
        let changed = handle_child_session_notification(update, "child-1", &mut agent, &oauth_auth(), None);
        assert!(!changed);
    }
