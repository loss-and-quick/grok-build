...
pub(super) fn handle_session_notification_with_origin(
    ...
    let changed = match session_notif.update {
        ...
        | XaiSessionUpdate::MemorySessionSaved { .. }) => {
            let changed = apply_compaction_or_retry_update(agent, update, &auth);
    ...
    match update {
        ...
        | XaiSessionUpdate::MemorySessionSaved { .. } => {
            ...
            if let Some(child_view) = agent.child_view_for_live_update_mut(child_sid) {
                changed = apply_child_view_session_event_with_auth(child_view, &update, auth);
            }
...
///
/// The from-disk replay knows only whether the process is on an API key — it
/// has no live login to name — so it enters through this shim. The live child
/// routing calls [`apply_child_view_session_event_with_auth`] with the
/// credential the session is actually running on, which is what lets a 401 in
/// a child name the thing that has to be repaired.
pub(crate) fn apply_child_view_session_event(
    child_view: &mut AgentView,
    update: &XaiSessionUpdate,
    is_api_key_auth: bool,
) -> bool {
    apply_child_view_session_event_with_auth(
        child_view,
        update,
        &SessionAuthContext {
...
/// [`apply_child_view_session_event`] against the session's own credential.
pub(crate) fn apply_child_view_session_event_with_auth(
    child_view: &mut AgentView,
    update: &XaiSessionUpdate,
    auth: &SessionAuthContext,
) -> bool {
    apply_compaction_or_retry_update(child_view, update, auth)
}
fn apply_compaction_or_retry_update(
    ...
    let changed = apply_session_event(update, &mut agent.session, &mut agent.scrollback, auth);
    ...
    match update {
        ...
        XaiSessionUpdate::AutoCompactCompleted {
            ...
            if session.loading_replay || session.state.is_switch_model_compact() {
                ...
                // so this is announced straight away. Live compactions defer
                // instead, because the confirmed `used` for the turn arrives
                // after this notification and supersedes `tokens_after`.
                let event = session.record_compaction(*tokens_before, *tokens_after, *elapsed_ms);
    ...
    match retry {
        ...
        RetryState::Exhausted {
            ...
            if is_credit_limit {
                ...
                let error = crate::app::effects::sanitize_user_error(&copy);
