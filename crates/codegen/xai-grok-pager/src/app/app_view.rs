...
impl AppView {
    ...
    /// centrally, an API key has no consumer billing surface at all, and a
    /// backend-billed account is billed by the agent's backend itself.
    fn expect_usage_visible(&self) -> bool {
        self.team_name.is_none()
            && !self.is_api_key_auth
            && !self.backend_billed
            && self.is_first_party_account
    }
