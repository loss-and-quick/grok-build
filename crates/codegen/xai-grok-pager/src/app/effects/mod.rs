                            ...
                            TaskResult::WithPinnedMemoryMode {
                                ...
                                result: Box::new(TaskResult::SessionCreated {
                                    ...
                                    scheduler_background_loops:
                                        parse_session_scheduler_background_loops(resp.meta.as_ref()),
                                }),
                            }
                        }
                        Err(e) => {
...
fn session_info_fields(
    ...
    // Always reported, both ways round: writeback is switched on by a
    ...
    // it. An absent row would be indistinguishable from a row the user
    // missed, so the local case states itself too.
    push(
        "Transcript",
        if info.syncs_to_backend {
            "synced to your grok.com account".to_string()
        } else {
            "stored on this machine only".to_string()
        },
        true,
    );
