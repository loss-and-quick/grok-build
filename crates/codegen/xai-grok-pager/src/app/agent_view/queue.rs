...
impl AgentView {
    ...
    /// A server top row sends only when the row says the agent will take it (its `editable` answer, else its wire kind).
    ...
    pub(crate) fn held_queue_top_sendable(&self) -> bool {
        ...
        if let Some(top) = self.shared_queue.iter().find(|entry| {
            ...
            return ServerRowCapabilities::for_wire(top, mutation).can_send_now();
        }
    ...
    pub(in crate::app) fn server_row_capabilities(
        ...
        self.shared_queue
            ...
            .map(|entry| ServerRowCapabilities::for_wire(entry, mutation))
    }
    ...
    fn server_queue_reordered(&self, selection_id: u64, up: bool) -> Option<Vec<String>> {
        ...
        let can_reorder = |id: &str| {
            self.shared_queue
                ...
                .is_some_and(|entry| ServerRowCapabilities::for_wire(entry, mutation).can_reorder())
        };
