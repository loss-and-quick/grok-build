...
impl AgentView {
    ...
    /// Answers from mounted review / KeptPlan / the agent-reported plan body without reading a file.
    fn plan_preview_available(&self) -> bool {
        ...
        // The saved plan as the agent reported it (`x.ai/session/plan`), never
        // a read of `plan.md` off this machine: a browser cannot make that read.
        self.plan_file_body.is_some()
    }
