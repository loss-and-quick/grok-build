...
impl TaskEntry {
    ...
    fn from_scheduled(
        ...
        // A foreground fire runs as this session's own turn (`current_cron`) or waits in its
        // queue; a detached fire runs in the linked subagent.
...
mod tests {
    ...
    /// A foreground fire runs as this session's own turn, so the row reads
    /// (running) while that turn is the one in flight.
    #[test]
