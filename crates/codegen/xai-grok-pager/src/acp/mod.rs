...
fn client_capabilities_meta(flags: &ConnectFlags) -> serde_json::Value {
    ...
    if let Some(obj) = meta.as_object_mut() {
        ...
        obj.insert(
            "x.ai/folderTrust".into(),
            serde_json::json!({ "interactive": flags.interactive_trust }),
        );
    }
    meta
}
...
pub(crate) struct InitializedAgent {
    ...
    /// See [`parse_restored_auth_meta`].
    pub(crate) restored_auth_meta: Option<serde_json::Value>,
}
