...
impl AcpUpdateTracker {
    ...
    /// The scrollback event announcing a completed compaction.
    ///
    /// Both paths that surface a compaction — the deferred flush in
    /// [`Self::finish_turn`] and the replay path that renders a resumed
    /// session's past compactions — go through here.
    ///
    /// No history accumulates behind it. The session's compaction history is
    /// the agent's, read back from the session log and carried on
    /// `x.ai/session/info`, so it is the same for a client that watched these
    /// events and one that attached after them — which a count kept here
    /// could never be.
