...
- `queue` lands in the subagent's queue: a running subagent receives it in the current turn at its next response boundary (after its next model response), rather than at the next safe point.
