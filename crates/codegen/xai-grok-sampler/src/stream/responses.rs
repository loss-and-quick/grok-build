...
pub(crate) fn stream_responses_tracked<'a>(
    ...
    async_stream::stream! {
        ...
        let (stop_reason, raw_stop_reason) = if has_tool_calls {
            ...
            // A refusal is reported on a *completed* response, so the status
            // alone cannot distinguish it from a normal turn: without this the
            // refusal surfaced as `Stop` with empty content, tripping the
            // empty-response retry loop instead of the shell's refusal notice.
            // Tool calls still win: a turn the caller must run tools for is
            // never reported as filtered.
