...
use xai_grok_sampling_types::{
    ...
    SentCredential, build_gemini_request, build_messages_request, is_check_event, messages, rs,
};
...
///
/// The other stream paths get this from `StreamSpanTiming::record_transport_failure`;
/// the Gemini path carries no `StreamSpanTiming`, so it records on the current span.
...
impl SamplingClient {
    ...
    pub fn new(config: SamplerConfig) -> Result<Self> {
        ...
        let http = if let Some(proxy) = config.proxy.as_deref() {
            ...
            // never contaminates the process-wide shared clients. The proxied
            // client carries no client certificate, so a model that sets both
            // `mtls_cert_dir` and a provider proxy goes out without mTLS.
            if config.mtls_cert_dir.is_some() {
                tracing::warn!(
                    "mtls_cert_dir is ignored for a model routed through a provider proxy"
                );
            }
            tracing::info!(
        ...
        } else if let Some(cert_dir) = config.mtls_cert_dir.as_deref() {
    ...
    pub async fn chat_completion(
        ...
        let (built_request, sent_bearer) = if self.has_request_interceptor() {
